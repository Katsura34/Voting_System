...
<div class="card shadow-sm rounded mx-auto my-5" style="max-width: 530px;">
    <div class="card-body">
        <h1 class="h4 fw-semibold text-secondary mb-4">Edit Candidate</h1>
        <form method="POST" action="{{ route('candidates.update', $candidate) }}" enctype="multipart/form-data">
           ...
           <div class="mb-4">
                ...
                <select name="position_id"
                    ...
                    @foreach($positions as $position)
                        <option value="{{ $position->id }}"
                            {{ $candidate->position_id == $position->id ? 'selected' : '' }}>
                            {{ $position->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                <label class="form-label text-secondary">Current Photo</label><br>
                @if($candidate->photo)
                ...
                @else
                    <p class="text-secondary">No photo uploaded</p>
                @endif
            ...
            <div class="mb-4">
                ...
                <textarea name="description"
                    ...
                    rows="3">{{ old('description', $candidate->description) }}</textarea>
                @error('description')
                    <div class="invalid-feedback d-block">{{ $message }}</div>
                @enderror
            </div>
